...
def render_rows(title, code, rows, spark=False):
    html = f'<div class="card"><div class="panel-bar"><span>{title}</span><span class="go-code">{code} &lt;GO&gt;</span></div><div class="panel-body">'
    for r in rows:
        s = svg_spark(r.get("spark", [])) if spark else ""
        html += f"""<div class="row-line">
            <span class="rlabel">{r['label'].upper()}</span>
            <span class="rvalue">{fmt_price(r['price'])}</span>
            <span class="rchg {cls(r['change_pct'])}">{fmt_pct(r['change_pct'])}</span>
            {s}
        </div>"""
    html += "</div></div>"
...
with st.sidebar:
    ...
    for key, label in nav:
        st.markdown(f"""<div style='padding:4px 0;font-size:11px;color:#ccc'>
        <span style='color:#ff9900;font-weight:700'>{key}</span> {label}</div>""", unsafe_allow_html=True)
...
for col, item in zip(fcols, config.FLOWS_PLACEHOLDER):
    col.markdown(f"""<div style="text-align:center"><div style="color:#888;font-size:9.5px;text-transform:uppercase">{item['label']}</div>
    <div class="{item['dir']}" style="font-size:12px;font-weight:700">{item['value']}</div></div>""", unsafe_allow_html=True)
panel_close()
...
with c4:
    ...
    for i, sector in enumerate(heatmap):
        ...
        with hcols[i % 4]:
            st.markdown(f"""<div class="sector-tile" style="background:{bg}">
            <div style="font-size:9.5px;color:#ccc;text-transform:uppercase">{sector['label']}</div>
            <div class="{cls(c)}" style="font-size:11px;font-weight:700">{fmt_pct(c)}</div></div>""",
            unsafe_allow_html=True)
    panel_close()
...
with c5:
    ...
    for m in config.MACRO_PLACEHOLDER:
        st.markdown(f"""<div class="row-line"><span class="rlabel">{m['label'].upper()}</span>
        <span class="rvalue" style="font-size:11px">{m['value']}</span>
        <span class="{m['dir']}" style="width:48px;text-align:right;font-size:10px">{m['chg']}</span></div>""",
        unsafe_allow_html=True)
    panel_close()
